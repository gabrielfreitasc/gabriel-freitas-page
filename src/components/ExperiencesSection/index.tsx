...
export function ExperiencesSection() {
  return (
    <section className="relative w-full flex flex-col items-center px-8">
      ...
      <div className="relative w-full min-h-screen flex flex-col gap-32">
        {slides.map(({ id, component: Component }, index) => (
          <div
            key={id}
            className="w-full h-full relative odd:left-[-2.5%] even:right-[-2.5%]"
          >
            <Component />
          </div>
        ))}
